...
class Menu(menus.base_menu.BaseMenu):
    ...
    def get_selection(self):
        ...
        while input_key not in [ord('\n'), curses.KEY_RIGHT]:
            ...
            self.screen.addstr(pos, 2, 'awspm > ... > Remove a role', curses.A_BOLD)
            pos += 1
            self.screen.addstr(pos, 2, '===========================', curses.A_BOLD)
            pos += 2
            self.screen.addstr(pos, 2, 'Select a role to remove:')
            pos += 1
            ...
            for key in sorted(self.config.data['key_pairs'].keys()):
                if 'roles' not in self.config.data['key_pairs'][key]:
                    continue

                for role in sorted(self.config.data['key_pairs'][key]['roles']):
                    color = curses.color_pair(1) if self.selected_option == role_count + 1 else curses.A_NORMAL
                    self.screen.addstr(pos, 2, '{0:s}-{1:s}'.format(key, role, self.config.data['key_pairs'][key]['iam_username']), color)
                    role_count += 1
                    option_count += 1
                    pos += 1
